...
SEED            = 420
EPOCHS          = 100
BATCH_SIZE      = 8   # increase effective batch size
IMG_H, IMG_W    = 720, 720
OUTPUT_SIZE     = IMG_H // 4
...
class CenterNetFPN(nn.Module):
    ...
    def forward(self, x):
        ...
        return (
            torch.sigmoid(self.hm(feat)),  # heatmap
            self.wh(feat),                 # width/height
            self.off(feat),                # offset
            self.cls(feat)                 # raw logits for classification
        )
