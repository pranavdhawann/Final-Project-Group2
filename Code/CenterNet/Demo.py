...
def draw_gaussian(hm, center, r):
    ...
    # Skip if center is far outside the heatmap
    ...
    # Skip if valid region is empty
    if left >= right or top >= bottom:
        return

    # Calculate gaussian patch coordinates
    g_left = max(0, -x + r)
    g_right = g_left + (right - left)
    g_top = max(0, -y + r)
    g_bottom = g_top + (bottom - top)

    # Apply gaussian to heatmap
...
def create_centernet_targets(boxes, debug=False):
    """Create CenterNet target maps from bounding boxes.
    ...
    Returns:
        ...
        size: Width/height map for object dimensions
        off: Offset map for center sub-pixel location
        mask: Mask indicating valid object locations
    """
    ...
    for box_idx, (x, y, w, h) in enumerate(boxes):
        ...
        # Check if center is within output bounds
        ...
        # Integer center coordinates
        xi, yi = int(cx_out), int(cy_out)

        # Gaussian radius based on box size (use at least radius 2 for better visibility)
        ...
        # Set width, height at center
        size[:, yi, xi] = [w, h]

        # Set offset (sub-pixel location)
        off[:, yi, xi] = [cx_out - xi, cy_out - yi]

        # Set mask (valid location)
        mask[yi, xi] = 1
...
def preprocess_for_centernet(img, device):
    ...
    # Ensure grayscale image is converted to 3 channels
    if len(img.shape) == 2:
...
def main():
    ...
    if model_name == "YOLOv10":
        ...
        with st.spinner(f'Running {model_name} detection...'):
            if model_name == "CenterNet":
                # Use the specialized preprocessing for CenterNet
                tensor = preprocess_for_centernet(arr, model.device)
                out = model(tensor)
